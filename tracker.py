...
TIME_BETWEEN_UPDATES = dt.timedelta(seconds=10)
...
async def main() -> None:
    ...
    last_tle_update = loader.get_timestamp()
    ...
    while True:
        ...
        while (len(task_list) < MAX_AWAITABLE_PASSES):
            ...
            candidates[0].handle_candidate_pass()
            
            # Filtramos para evitar solapamientos
            filtered_passes = filter_overlapping_passes(candidates[:1]+filtered_passes+recovered_passes, track_list)
            recovered_passes=[]
            ...
            for p in filtered_passes:
                # Se verifica que la pasada no tenga un worker ya creado
                if p.task == None :
        ...
        for task in done:
            sat_p=task_to_pass_map.get(task)
            try:
                await task
                ...
                if task in task_to_pass_map: del task_to_pass_map[task]
                if sat_p and sat_p in filtered_passes: filtered_passes.remove(sat_p)
